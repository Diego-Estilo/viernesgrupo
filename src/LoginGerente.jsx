...
export default function LoginGerente({ modoOscuro }) {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: modoOscuro ? '#0f172a' : '#f1f5f9', position: 'relative', overflow: 'hidden', fontFamily: "'Inter', system-ui, sans-serif", transition: 'background 0.3s ease' }}>
      ...
      <div style={{ position: 'absolute', top: '-10%', left: '-10%', width: '500px', height: '500px', background: modoOscuro ? 'radial-gradient(circle, rgba(59,130,246,0.3) 0%, rgba(59,130,246,0) 70%)' : 'radial-gradient(circle, rgba(59,130,246,0.1) 0%, rgba(59,130,246,0) 70%)', borderRadius: '50%', filter: 'blur(60px)', animation: 'float 10s infinite ease-in-out alternate' }}></div>
      <div style={{ position: 'absolute', bottom: '-15%', right: '-5%', width: '600px', height: '600px', background: modoOscuro ? 'radial-gradient(circle, rgba(236,72,153,0.25) 0%, rgba(236,72,153,0) 70%)' : 'radial-gradient(circle, rgba(236,72,153,0.08) 0%, rgba(236,72,153,0) 70%)', borderRadius: '50%', filter: 'blur(80px)', animation: 'float 12s infinite ease-in-out alternate-reverse' }}></div>
      ...
      <div style={{ maxWidth: '440px', width: '90%', padding: '48px', borderRadius: '28px', background: modoOscuro ? 'rgba(255, 255, 255, 0.03)' : 'rgba(255, 255, 255, 0.8)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: modoOscuro ? '1px solid rgba(255, 255, 255, 0.1)' : '1px solid rgba(0, 0, 0, 0.05)', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.2)', position: 'relative', zIndex: 10 }}>
        ...
        <h1 style={{ fontSize: '2rem', color: modoOscuro ? '#ffffff' : '#0f172a', fontWeight: '800', textAlign: 'center', margin: '0 0 8px 0', letterSpacing: '-0.5px' }}>Portal Gerencial</h1>
        <p style={{ marginBottom: '40px', color: modoOscuro ? '#94a3b8' : '#64748b', fontSize: '1rem', textAlign: 'center' }}>Inicia sesión para continuar</p>
